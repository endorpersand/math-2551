...
**Unit Tangent Vector**: The unit vector that is tangent to the curve
$$
\Large
\vec{T}(t) = \frac{\vec{r}'(t)}{\|\vec{r}'(t)\|} = \frac{\vec{v}(t)}{\|\vec{v}(t)\|} = \frac{d\vec{r}/dt}{ds/dt}$$
(This is the same as normalizing $\vec{v}$)
...
(This is the same as normalizing $T'(t)$, and is $\perp$ to $\vec{T}$)
...
$$
*This formula is derived in more advanced texts, so no explanation here.*

### Additional Formulas for Curvature
$$
\Large
\begin{align*}
